...
Refatorado: constantes → cerco_setup_state_data.py
"""
...
from ...config import (
    ESTADO_JOGO_MENU_PRINCIPAL, ESTADO_JOGO_CERCO,
    LARGURA_TELA, ALTURA_TELA, RESOLUCOES, atualizar_resolucao,
)
from .data import (
    C_BG, C_PAINEL, C_BORDA, C_ACENTO, C_OURO, C_TEXTO, C_DIM, C_VERDE, C_PERIGO,
    DIFICULDADES, HEROIS_DISPONIVEIS,
)
    ...
    def _draw_herois(self, tela, W):
        ...
        for i, (nome, _, classe) in enumerate(HEROIS_DISPONIVEIS):
            ...
            if nome == "Aquele":
                ...
                tela.blit(fallback, (avatar_rect.centerx - fallback.get_width() // 2,
                                     avatar_rect.centery - fallback.get_height() // 2))
            else:
